...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    for i, (imgs, targets, paths, _, _, lable) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with amp.autocast(enabled=enable_amp):
            ...
            loss_dict["att_loss"] = levelAttention_loss_focalloss(mask, lable) # sigmod
            #loss_dict["att_loss"] = levelAttention_loss2(mask, lable)  # tanh
            #loss_dict["mse_loss"] = levelAttention_mse(mask, lable)
            #loss_dict["att_loss"] = loss_a+loss_b
            #b = levelAttention_loss4(mask, lable)
            #loss_dict["att_loss"] = a+b
            ...
            loss_items = torch.cat((loss_dict_reduced["box_loss"],
                                    ...
                                    loss_dict_reduced["att_loss"],
                                    #loss_dict_reduced["mse_loss"],
...
def evaluate(model, data_loader, coco=None, device=None):
    ...
    for imgs, targets, paths, shapes, img_index, _ in metric_logger.log_every(data_loader, 100, header):
        imgs = imgs.to(device).float() / 255.0  # uint8 to float32, 0 - 255 to 0.0 - 1.0
        # targets = targets.to(device)
