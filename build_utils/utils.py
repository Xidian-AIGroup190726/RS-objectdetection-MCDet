...
def levelAttention_loss1(attention_mask, bboxs):
    h, w = 1, 1
    mask_losses = []
    batch_map = []
    ...
    for j in range(batch_size):
        ...
        bbox_annotation = bboxs[j][:, 2:]
        #bbox_annotation = bbox_annotation[bbox_annotation[:, 4] != -1]

        cond1 = torch.le(bbox_annotation[:, 0], w)  # 判断是否小于等于
        ...
        if bbox_annotation.shape[0] == 0:
            mask_losses.append(torch.tensor(0).float().cuda(2))
            continue

        bbox_area = (bbox_annotation[:, 2]) * (bbox_annotation[:, 3])
        ...
        for id in range(len(attention_mask)):
            ...
            min_area = 32*32/(416*416)  # (2 ** (id + 5)) ** 2 * 0.5
            max_area = 96*96/(416*416)  # (2 ** (id + 5) * 1.58) ** 2 * 2

            level_bbox_indice1 = torch.ge(bbox_area, min_area)  # 即是否 input>=最小区域 中，大
            level_bbox_indice2 = torch.le(bbox_area, max_area)  # 即是否 input<=最大区域 中，小

            level_bbox_indice = level_bbox_indice1 * level_bbox_indice2 # 若True, 即为中目标。 False,小目标或大目标
            level_bbox_indice_big = torch.ge(bbox_area, max_area)
            level_bbox_indice_lit = torch.le(bbox_area, min_area)

            if id==0:
                level_bbox_indice = level_bbox_indice_big
            if id==1:
                level_bbox_indice = level_bbox_indice
            if id == 2:
                level_bbox_indice = level_bbox_indice_lit
            copy_bbox_annotation = bbox_annotation[level_bbox_indice, :]
            ...
            if level_bbox_annotation.shape[0]: # xywh->x1y1x2y2
                #copy_bbox_annotation = level_bbox_annotation
                level_bbox_annotation[:, 0] = (copy_bbox_annotation[:, 0]-copy_bbox_annotation[:, 2]/2)* (attention_w / w)  # x1
                level_bbox_annotation[:, 1] = (copy_bbox_annotation[:, 1]-copy_bbox_annotation[:, 3]/2)* (attention_h / h)  # y1
                level_bbox_annotation[:, 2] = (copy_bbox_annotation[:, 0]+copy_bbox_annotation[:, 2]/2)* (attention_w / w)  # x2
                level_bbox_annotation[:, 3] = (copy_bbox_annotation[:, 1]+copy_bbox_annotation[:, 3]/2)* (attention_h / h)  # y2
            ...
            mask_predict = attention_map.float().cuda(2)#[attention_map >= 0]

            mask_loss.append(torch.nn.functional.binary_cross_entropy_with_logits(mask_predict, mask_gt))
        mask_losses.append(torch.stack(mask_loss).mean())

    return torch.stack(mask_losses).mean(dim=0, keepdim=True).float().cuda(2)
...
def levelAttention_loss_focalloss(attention_mask, bboxs): # focal_loss
    h, w = 1, 1
    devices = attention_mask[0].data.device
    mask_losses = []
    batch_map = []
    batch_size = len(bboxs)
    for j in range(batch_size):
        ...
        if bbox_annotation.shape[0] == 0:
            mask_losses.append(torch.tensor(0).float().to(devices))
            continue
        ...
        for id in range(len(attention_mask)):
            ...
            level_bbox_indice_med = level_bbox_indice1 * level_bbox_indice2
            level_bbox_indice_big = torch.ge(bbox_area, max_area)
            level_bbox_indice_lit = torch.le(bbox_area, min_area)
            level_bbox_indice_all = torch.ge(bbox_area, 0)
            level_bbox_indice_none = torch.le(bbox_area, -1)

            if id==0:
                level_bbox_indice = level_bbox_indice_big
                level_bbox_mask = level_bbox_indice_none
            if id==1:
                level_bbox_indice = level_bbox_indice_med
                level_bbox_mask = level_bbox_indice_none
            if id == 2:
                level_bbox_indice = level_bbox_indice_lit
                level_bbox_mask = level_bbox_indice_none
            ...
            copy_mask_annotation = bbox_annotation[level_bbox_mask, :]
            mask_annotation = bbox_annotation[level_bbox_mask, :].clone()

            if level_bbox_mask.shape[0]:  # xywh->x1y1x2y2
                mask_annotation[:, 0] = (copy_mask_annotation[:, 0] - copy_mask_annotation[:, 2] / 2) * (
                            attention_w / w)  # x1
                mask_annotation[:, 1] = (copy_mask_annotation[:, 1] - copy_mask_annotation[:, 3] / 2) * (
                            attention_h / h)  # y1
                mask_annotation[:, 2] = (copy_mask_annotation[:, 0] + copy_mask_annotation[:, 2] / 2) * (
                            attention_w / w)  # x2
                mask_annotation[:, 3] = (copy_mask_annotation[:, 1] + copy_mask_annotation[:, 3] / 2) * (
                            attention_h / h)  # y2

            mask_mask = torch.zeros(attention_map.shape)
            mask_mask = mask_mask.to(devices)
            for i in range(mask_annotation.shape[0]):
                x1 = max(int(mask_annotation[i, 0]), 0)
                y1 = max(int(mask_annotation[i, 1]), 0)
                x2 = min(math.ceil(mask_annotation[i, 2]) + 1, attention_w)
                y2 = min(math.ceil(mask_annotation[i, 3]) + 1, attention_h)
                mask_gt[y1:y2, x1:x2] = 99

            mask_gt = mask_gt[mask_gt >= 0]
            mask_predict = attention_map[attention_map != 999]
            mask = (mask_gt <= 1)
            gt = torch.masked_select(mask_gt, mask)
            predict = torch.masked_select(mask_predict, mask)
            mask_loss.append(torch.nn.functional.binary_cross_entropy_with_logits(predict, gt))
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    latt = torch.zeros(1, device=device)  # Tensor(0)
    #lmse = torch.zeros(1, device=device)  # Tensor(0)
