...
class FeatureConcat(nn.Module):
    ...
    def forward(self, x, outputs):
        tupe = []
        if self.multiple:
            b,c = [outputs[i] for i in self.layers]
            if(b.size(2)==c.size(2)):
                ...
                tupe.append(c)
                return tupe
        else: a = outputs[self.layers[0]]

        #a = torch.cat([outputs[i] for i in self.layers], 1) if self.multiple else outputs[self.layers[0]]
        return a
