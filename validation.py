...
def main(parser_data):
    ...
    model.eval()
    with torch.no_grad():
        for imgs, targets, paths, shapes, img_index, _ in tqdm(val_dataset_loader, desc="validation..."):

            img_o = np.array(imgs.squeeze(0).permute(1,2,0))[:, :, ::-1]
            img = img_utils.letterbox(img_o, new_shape=(416, 416), auto=True, color=(0, 0, 0))[0]

            img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
            img = np.ascontiguousarray(img)
            img = torch.from_numpy(img).to(device).float()
            img /= 255.0  # scale (0, 255) to (0, 1)
            imgs = img.unsqueeze(0)  # add batch dimension

            t1 = torch_utils.time_synchronized()
            pred = model(imgs)[0]  # only get inference result
            t2 = torch_utils.time_synchronized()
            print(t2 - t1)
            pred = non_max_suppression(pred, conf_thres=0, iou_thres=0.5, multi_label=False)#[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weights', default='./weights/20230509-113944.txtbest.pt', type=str, help='training weights')#nspp2
    # batch size
